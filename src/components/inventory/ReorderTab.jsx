...
import { Download, Package, ShoppingCart, ExternalLink } from "lucide-react";
...
export default function ReorderTab() {
  ...
  const suggestedQty = (item) => {
    ...
    if (min > current) return (min - current) + min;
    return 10;
  };
      ...
      {itemsWithoutSku.length > 0 && (
        <Card className="bg-amber-50 border-amber-200 shadow-sm">
          <CardContent className="pt-4">
            ...
            <p className="text-xs text-amber-600 mt-2">Add an Item ID/SKU to include these in the Wurth order.</p>
          </CardContent>
        </Card>
      )}
    </div>
