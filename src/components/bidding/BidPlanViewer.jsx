      ...
      <DialogContent className="max-w-[96vw] w-[1400px] h-[95vh] max-h-[95vh] overflow-hidden flex flex-col p-0 gap-0">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {measurements.length > 0 && (
            <div className="w-60 border-l bg-white flex flex-col overflow-hidden flex-shrink-0">
              <div className="px-3 py-2.5 border-b bg-slate-50 flex items-center justify-between">
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-1.5"><Ruler className="w-4 h-4 text-emerald-600"/>Measurements</h3>
                <span className="text-xs text-slate-400">{measurements.length} saved</span>
              </div>
              <div className="flex-1 overflow-y-auto p-2 space-y-2">
                {measurements.map((m, idx) => (
                  <div key={m.id||idx} className="p-2.5 bg-emerald-50 rounded-lg border border-emerald-200">
                    <div className="flex items-start justify-between gap-1">
                      <div className="min-w-0 flex-1">
                        <p className="text-xs font-semibold text-slate-700 truncate leading-snug">{m.label}</p>
                        <p className="text-lg font-bold text-emerald-700 leading-tight">{m.realFeet!=null?`${m.realFeet.toFixed(2)} LF`:<span className="text-sm text-amber-600">No scale set</span>}</p>
                      </div>
                      <button onClick={()=>setMeasurements(p=>p.filter((_,i)=>i!==idx))} className="text-slate-300 hover:text-red-500 flex-shrink-0"><X className="w-3.5 h-3.5"/></button>
                    </div>
                    {rooms.length > 0 && m.realFeet != null && (
                      <button onClick={()=>{ setSendingM(m); setSendRoomId(rooms[0]?.id||""); setSendCategory("base"); }}
                        className="mt-1.5 w-full flex items-center justify-center gap-1 text-xs text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded px-2 py-1 transition-colors font-medium">
