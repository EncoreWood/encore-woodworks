...
export default function BidRoomSection({ room, catalogItems, categories, pricingConfigs, bidType, onChange, onDelete, sketchPaths = [], specs = {}, linkedProjectId = null, planAnnotations = [], planScalePxPerFt = null, onViewOnPlan }) {
  ...
  const handleUseAiEstimate = () => {
    const restored = room.ai_items_snapshot || null;
    onChange({ ...room, items: restored || room.items, pricing_source: "ai", ai_items_snapshot: null });
  };
  ...
  return (
    <Card className="overflow-hidden border border-slate-200">
      ...
      >
        ...
        {/* Price from Plan Marks toggle */}
        {room.pricing_source === "plan_marks" ? (
          <div className="flex items-center gap-1 flex-shrink-0" onClick={e => e.stopPropagation()}>
            <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 rounded-full whitespace-nowrap">Priced from Plan ✓</span>
            <Button variant="outline" size="sm" onClick={handleUseAiEstimate} className="h-7 px-2 text-[10px] text-slate-600 border-slate-300 whitespace-nowrap">Use AI</Button>
          </div>
        ) : (
