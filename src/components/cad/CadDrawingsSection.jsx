...
import { Upload, FileCode2, Trash2, Eye, Loader2, Calendar } from "lucide-react";
...
export default function CadDrawingsSection({ project, currentUser, onSave }) {
  ...
  const cadFiles = (project.files || []).filter(f => f.tag === "cad_dxf");
  ...
  const handleDelete = (idx) => {
    if (!confirm("Delete this DXF file?")) return;
    const updated = (project.files || []).filter((f, i) => {
      // find the actual index in project.files
      const cadIdx = (project.files || []).filter(ff => ff.tag === "cad_dxf").indexOf(f);
      return !(f.tag === "cad_dxf" && cadIdx === idx);
    });
  ...
  return (
    <>
        ...
        {cadFiles.length === 0 ? (
          ...
          >
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-slate-600">Upload DXF Files</p>
              <p className="text-xs text-slate-400 mt-1">Export from Mozaik and upload here. Supports multiple files (one per room).</p>
            </div>
          </div>
        ) : (
          <div className="space-y-2">
            {cadFiles.map((file, idx) => (
              <div key={idx} className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 hover:border-cyan-200 hover:bg-cyan-50/30 transition-all group">
                <FileCode2 className="w-8 h-8 text-cyan-500 flex-shrink-0" />
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  {currentUser?.role === "admin" && (
                    <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-red-400 hover:text-red-600 hover:bg-red-50" onClick={() => handleDelete(idx)}>
      ...
      {viewingFile && <DxfViewer file={viewingFile} onClose={() => setViewingFile(null)} />}
    </>
  );
}