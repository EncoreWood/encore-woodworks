...
export default function TrainingCategoryManager({ open, onOpenChange }) {
  ...
  const createMutation = useMutation({
    mutationFn: (name) => base44.entities.TrainingCategory.create({ name, sort_order: categories.length }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["trainingCategories"] }); setNewName(""); },
  });

  const renameMutation = useMutation({
    mutationFn: async ({ id, oldName, newName }) => {
      await base44.entities.TrainingCategory.update(id, { name: newName });
      if (oldName !== newName) {
        await base44.entities.Training.updateMany({ category: oldName }, { $set: { category: newName } });
      }
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.TrainingCategory.delete(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["trainingCategories"] }),
  });
  ...
  const reorderMutation = useMutation({
    mutationFn: async ({ id, direction }) => {
      ...
      await base44.entities.TrainingCategory.bulkUpdate([
