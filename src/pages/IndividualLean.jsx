...
export default function IndividualLean() {
  ...
  const { data: items = [] } = useQuery({
    queryKey: ["individualLeanItems"],
    queryFn: async () => {
      const user = await base44.auth.me();
      setCurrentUser(user);
      // Filter to only show current user's items
      const allItems = await base44.entities.IndividualLeanItem.list("-created_date", 500);
      return allItems.filter(item => item.created_by === user.email);
    }
  });

  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.IndividualLeanItem.create({
      ...data,
      category: selectedCategory
    }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["individualLeanItems"] });
      setShowForm(false);
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.IndividualLeanItem.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["individualLeanItems"] });
      setEditingId(null);
      setFormData({ title: "", status: "open", priority: "medium", assigned_to: "", due_date: "" });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.IndividualLeanItem.delete(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["individualLeanItems"] })
  });
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-slate-900 mb-1">Individual Lean Board</h1>
        <p className="text-slate-600 text-sm mb-6">Your personal lean items only</p>
              ...
              <div key={status} className="flex flex-col">
                ...
                <div className="space-y-3 flex-1">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 p-3 rounded-lg border-2 border-dashed border-slate-300 hover:border-blue-500 hover:bg-blue-50 transition-all text-slate-600 hover:text-blue-700 text-sm font-medium"
                  >
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={handleSubmit} className="bg-blue-600 hover:bg-blue-700" disabled={!formData.title.trim()}>
