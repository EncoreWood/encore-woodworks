...
export default function Education({ folder }) {
  ...
  const handleSave = (data) => {
    if (editingTraining) updateMutation.mutate({ id: editingTraining.id, data });
    else createMutation.mutate(data);
  };
  const handleEdit = (t) => { setEditingTraining(t); setShowForm(true); };
  const handleNew = () => { setEditingTraining(null); setShowForm(true); };

  const folderTrainings = trainings.filter((t) => (t.category || "") === folder);
  const filtered = folderTrainings.filter(
    (t) =>
      !searchQuery ||
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900">{folder} Training</h1>
            <p className="text-sm text-slate-500 hidden sm:block">Training materials for the {folder} area</p>
          </div>
          {isAdmin && (
        ...
        <div className="relative mb-4 max-w-md">
          ...
          <Input
            ...
            placeholder={`Search ${folder} trainings...`}
            className="pl-9"
          />
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 text-slate-400">
            ...
            <p className="font-medium">No {folder} trainings yet</p>
            {isAdmin && <p className="text-sm">Click "New Training" to add one.</p>}
          </div>
        ) : (
            ...
            {filtered.map((training) => (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-indigo-100 text-indigo-700">{folder}</span>
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {isAdmin && (
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button
                          onClick={() => { if (confirm("Delete this training?")) deleteMutation.mutate(training.id); }}
                ...
                {training.assigned_to?.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-slate-100">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {training.assigned_to.slice(0, 3).map((name) => (
                        <span key={name} className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{name}</span>
                      ))}
